    ...
    >
      <div className="max-w-7xl mx-auto flex items-center justify-between px-6 py-4">
        <NavLink to="" className="flex items-center">
          <svg
            className={`size-8 ${isScrolled ? "fill-[#3674B5]" : "fill-white"}`}
            viewBox="0 0 14 14"
            role="img"
            focusable="false"
            aria-hidden="true"
          ...
          <span
            className={`ml-2 text-xl font-bold ${
              isScrolled ? "text-[#3674B5]" : "text-white"
            }`}
          >
            airbnb
          </span>
              ...
              <NavigationMenuItem key={item.to}>
                <NavigationMenuLink className="hover:bg-transparent">
                  <NavLink
                    to={item.to}
                    className={({ isActive }) =>
                      `flex items-center gap-1 text-sm font-medium transition relative hover:text-[#3674B5]
        ...
        <div className="hidden md:flex items-center space-x-4">
          <div
            className={`flex items-center border rounded-full px-2 py-1 hover:shadow-md transition ${
              isScrolled ? "border-gray-300" : "border-white/50"
            }`}
          >
            <div className="w-6 h-6 bg-gray-500 rounded-full flex items-center justify-center mx-2">
      ...
      {mobileMenuOpen && (
        <div
          className={`md:hidden px-6 py-4 space-y-3 transition-all ${
            isScrolled ? "bg-white" : "bg-black/80 text-white"
          }`}
        >
          {navItems.map((item) => (
            <NavLink
